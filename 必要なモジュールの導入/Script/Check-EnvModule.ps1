...
function Check-EnvModule {
  ...
  # モジュールがインストール済みか確認
    ...
    if(!($cnt -eq 0)){
        ...
        foreach($ver in $getVer){
            if($ver.ToString() -eq $ModuleVersion){
                ...
                Write-CommonLog -Message ("INFO: "+(Get-Date -Format "yyyyMMdd HH:mm:ss")+" [EXIST] "+$ModuleName+" Version: "+$ver.ToString()) -LogPath $Log -Level 'INFO'
            }else{
                # Write-Log -Message ("INFO: "+(Get-Date -Format "yyyyMMdd HH:mm:ss")+" [OTHER] "+$ModuleName+" Version: "+$ver.ToString()) -LogPath $Log
                Write-CommonLog -Message ("INFO: "+(Get-Date -Format "yyyyMMdd HH:mm:ss")+" [OTHER] "+$ModuleName+" Version: "+$ver.ToString()) -LogPath $Log -Level 'INFO'
            }
        }
    }else{
        # Write-Log -Message ("WARN: "+(Get-Date -Format "yyyyMMdd HH:mm:ss")+" [NOTHING] "+$ModuleName) -LogPath $Log
        Write-CommonLog -Message ("WARN: "+(Get-Date -Format "yyyyMMdd HH:mm:ss")+" [NOTHING] "+$ModuleName) -LogPath $Log -Level 'WARN'
    }
    ...
    if($flg -eq $false){
        ...
        Write-CommonLog -message ("WARN: "+(Get-Date -Format "yyyyMMdd HH:mm:ss")+" [INSTALL] "+$ModuleName+" Version: "+$ModuleVersion.ToString()) -LogPath $Log -Level 'WARN'
