...
- **Write-CommonLog.ps1:** Common フォルダに存在
- **getILSpyCmd.yaml:** YAML フォルダに存在
- **.NET SDK インストーラー:** 指定フォルダに存在
- **インターネット接続:** NuGet.org へのアクセス確認用
- **管理者権限:** .NET SDK インストール時に必須（SDK 未インストール時のみ）

## ログ出力

ログファイルは `LOG` フォルダに以下の形式で保存されます：
...
- **WARN:** 警告（処理は継続）
- **ERROR:** エラー（処理が中断）
- **DEBUG:** デバッグ情報（YAML フィールド検証時など）

## トラブルシューティング

### エラー: "SDKインストーラーが見つかりません"
→ `YAML` フォルダに `getILSpyCmd.yaml` が存在し、`DotnetSdk.SdkFolder` と `DotnetSdk.Installer` が正しく設定されているか確認してください。

### エラー: "管理者権限が必要です"
→ PowerShell を右クリック → "管理者として実行" を選択し、再度実行してください。

### エラー: "ネットワークエラー"
→ インターネット接続を確認し、NuGet.org に接続できるか確認してください。
...
### v1.3.0 (現在)
- **-NoKeyWait パラメータを追加** - スケジューラー/自動化環境での非対話実行に対応
...
このスクリプトは GitHub リポジトリの一部として提供されます。
