...
ユーザーの要望に基づき、**改善 #3（例外タイプのログレベル分類化）**と**改善 #4（パラメータ検証の強化）**を完全に実装しました。
...
**目的:** EnvYaml パラメータの柔軟性向上（相対パス対応、デフォルト構築）により、スクリプト呼び出し時の使いやすさを向上させる
...
| 指定方法 | 説明 | 例 |
|---------|------|-----|
| ファイル名のみ | YAML フォルダの直下を想定 | `.\getILSpyCmd.ps1 -EnvYaml "custom.yaml"` |
...
| デフォルト | YAML フォルダの getILSpyCmd.yaml | `.\getILSpyCmd.ps1` |
...
| バージョン | 日付 | 改善内容 |
|-----------|------|--------|
| **v1.4.0** | 2025-01-15 | 例外分類化、パラメータ検証強化 |
| v1.3.0 | 2025-01-15 | Exit 排除、-NoKeyWait パラメータ |
...
### 推奨される次の改善
1. **タイムアウト時間のパラメータ化** (#5 予定)
   - `-TimeoutSeconds` パラメータ追加
   - デフォルト: 600秒（10分）

2. **ロールバック機能のオプション化** (#6 予定)
   - `-AllowRollback` フラグ
   - ロールバック動作の詳細制御
