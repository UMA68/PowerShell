...
if (-not (Test-NoDoubleActivation -Thread "StringDecryption" -ShowDialog)) {
    # 既に起動中のため処理を終了
...
# SecureString の破棄
if ($null -ne $SecureDecryptedString) {
    $SecureDecryptedString.Dispose()
}

# 平文パスワードの削除
if ($null -ne $DecryptedString) {
