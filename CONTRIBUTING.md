...
**必須**: [SCOPE_GUIDELINES.md](SCOPE_GUIDELINES.md) にしたがってください。
...
- タグは「公開の節目」を示すものであり、変更履歴そのものではありません。
- すべての変更でタグを作成しません。
- タグは原則として `main` の安定点にのみ付与します。
- Release Notesは更新判断のための案内板とし、実装詳細やgit logの羅列は記載しません。
- 詳細はADR-0016を正本として参照してください：`ADR-0016: Release & Tagging Policy (docs/adr/0016-release-and-tagging-policy.md)`

## コミットメッセージ規約

[Conventional Commits](https://www.conventionalcommits.org/) にしたがってください。
...
# テストの追加
git commit -m "test(common): Write-CommonLogのユニットテストを追加"
...
- [ ] ユニットテストを追加・更新している（該当する場合）
- [ ] すべてのテストが通過している
- [ ] 適切なコメント・ドキュメントを追加している
- [ ] [SCOPE_GUIDELINES.md](SCOPE_GUIDELINES.md) にしたがっている
- [ ] 新機能の場合、使用例を追加している
- [ ] 既存の機能を壊していない
...
# 特定のテストを実行
Invoke-Pester -Path .\Tests\Common\Write-CommonLog.Tests.ps1

# カバレッジレポートを生成
Invoke-Pester -Path .\Tests\ -CodeCoverage .\Common\*.ps1 -CodeCoverageOutputFile .\coverage.xml
