...
.SYNOPSIS
    ...
    ドライランモード（-WhatIf）でも常にログファイルに記録します。
...
.DESCRIPTION
    ...
    - -WhatIf 対応：ドライランモードでも必ずログをファイルに書き込み（変更を伴わない操作）
    
    ログメッセージ形式:
    yyyy-MM-dd HH:mm:ss [LEVEL] - Message
    
    重要: ログ書き込みは -WhatIf パラメータでも実行されます。これにより、ドライラン時の
    実行計画をログに記録し、後で検証・監査できます。ShouldProcess チェックは行わないため、
    意図しない実行を防ぎたい場合は呼び出し側で制御してください。
...
.NOTES
    ...
    実装の詳細:
    - ログ書き込み処理は ShouldProcess を使用しないため、-WhatIf モードでも実行される
    - Add-Content コマンドに -WhatIf:$false を明示指定して、ドライランモード時の実行を保証
    - ファイルロック時の自動リトライ（3回まで、100ms 間隔）により、並行実行時の競合を軽減
    - マスキング処理と正規表現を用いた機密情報保護
    
    変更履歴:
    v1.3.1 (2026-01-16)
        - -WhatIf モード対応を修正：ShouldProcess を削除し、ログ書き込みを常に実行
        - Add-Content に -WhatIf:$false を明示指定
        - ドライランモードでもログがファイルに記録されるよう改善
    ...
    - ShouldProcess は使用しないため、-WhatIf モードでもログ書き込みが実行されます
    - 呼び出し側で -WhatIf:$false を指定してログ関数を呼び出す必要はありません
