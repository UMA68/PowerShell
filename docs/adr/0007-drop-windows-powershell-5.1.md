...
5. GitHub Actionsはwindows-latest同梱の最新pwshで実行し、PowerShell 7.x（原則として最新バージョンのみ）をテスト対象とする
   - `powershell/setup-powershell` は使用しない
6. ローカルの検証もPowerShell 7.x（原則として最新バージョン）を対象とする
7. 必要に応じて、将来PowerShell 5.1専用リポジトリを新設する
8. PowerShellの新バージョンが出た場合は手動で更新する
9. 5.1は技術負債を隔離するため、将来必要になったときに専用リポジトリで対応する
...
- 複数の7.xバージョンでのテストを理想としたが、検証の結果、運用コストに見合わないため最新1バージョンに絞った
...
- GitHub ActionsのPesterワークフローはwindows-latest同梱の最新pwshで実行する
- ローカル検証はPowerShell 7.x（原則として最新バージョン）を前提に手順を明文化する
