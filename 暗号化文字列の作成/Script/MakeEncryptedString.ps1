...
.EXAMPLE
    ...
    1. 二重起動チェックを実行します（既に起動中の場合は終了）
    2. 鍵ファイルを読み込みます
    3. 暗号化する文字列の入力を求められます
    4. 入力文字列をターミナルに表示します
    5. 入力文字列を SecureString に変換し暗号化します
    6. 暗号化文字列をターミナルに表示します
    7. 出力ファイル名の入力を求められます
    8. 指定ファイル名でスクリプト親ディレクトリに保存します
    9. ダイアログで完了を通知します
    10. 変数をクリーンアップして終了します
...
# このスクリプトの目的は暗号化文字列を作成することなので、-AsPlainTextの使用は正当
