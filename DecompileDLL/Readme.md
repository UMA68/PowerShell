...
- **新DLL検索フォールバック**: 完全一致がない場合、`<BaseName>*.dll`から候補を選択
...
│   ├── Old/                   # 旧バージョンDLL配置先（入力）
│   ├── New/                   # 新バージョンDLL配置先（入力）
│   └── Decompiled/            # 逆コンパイル結果出力先（Folders.Decompile で変更可）
│       ├── old/               # 旧DLLの逆コンパイル結果（Folders.Old）
│       └── new/               # 新DLLの逆コンパイル結果（Folders.New）
└── LOG/                       # 既定ログ出力先（LOG.Path で変更可）
```

> 補足: 入力フォルダーは `Dlls/Old` と `Dlls/New` 固定です。YAMLの `Folders.Old` / `Folders.New` は**出力側サブフォルダー名**を制御します。
...
**注意**: `-NoKeyWait`は処理完了後のキー入力待ちをスキップするためのオプションです。差分ツールの自動起動可否は `-DiffTool` と `-Parallel` に依存します（`-Parallel`時は自動起動しません）。
...
- `-ShowConfig` 指定時はYAML内容を表示して処理を終了します（逆コンパイルは実行しません）。
- 終了コードは `ExitCodes.Success`（既定: `0`）です。
...
# 出力フォルダー名設定（入力フォルダー Dlls/Old, Dlls/New は固定）
Folders:
...
# WinMergeインストール先（フォルダー）
InstWinMerge:
    Win10: C:\Program Files\WinMerge
    Win11: $HOME\AppData\Local\Programs\WinMerge
```
...
| セクション | キー | 用途 |
|-----------|------|------|
| `LOG` | `Path` / `FILENAME` / `EXTENSION` | 実行ログ出力先とログファイル名 |
| `Folders` | `Decompile` | 出力ルート（`Dlls/<Decompile>`） |
| `Folders` | `Old` / `New` | 出力サブフォルダー（`old` / `new` 等） |
| `Retry` | `MaxAttempts` / `DelaySeconds` / `TimeoutSeconds` | 逆コンパイル時のリトライ・待機・タイムアウト |
| `ExitCodes` | `Success` / `GeneralError` / `OSNotSupported` / `FileNotFound` / `DecompileFailed` | スクリプトが参照する終了コード |
| `Colors` | `Info` / `Success` / `Warning` / `Error` | UIメッセージ色 |
| `OSDetection` | `Win11MinBuild` / `Win10MinBuild` | 対応OS判定のしきい値 |
| `InstWinMerge` | `Win10` / `Win11` | WinMergeインストール先フォルダー |

> `ExitCodes.UserCancelled` は YAML に定義可能ですが、現行スクリプトでは参照していません。
...
- ファイル名が一致しているか確認（完全一致でない場合は部分一致候補から1件を選択します）
...
```Shell
<LOG.Path>/<LOG.FILENAME>_YYYYMMDD-HHMMSS<LOG.EXTENSION>
```

既定値では次のようになります。

```Shell
LOG/Report(dll)_YYYYMMDD-HHMMSS.log
```
...
※ 実際の値は `YAML/Decompile.yaml` の `ExitCodes.*` で変更可能です（表は既定値）。
...
| `-WhatIf` | switch | - | 共通パラメーター（実行せずに処理計画を確認） |
| `-Verbose` | switch | - | 共通パラメーター（詳細ログを表示） |
...
1. ログファイル（`<LOG.Path>/`フォルダー）
2. エラーレポート（`<LOG.Path>/DecompileErrors_*.txt`）
