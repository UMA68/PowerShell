...
process{
    ...
    if(!$Yaml.PowerShell.Version -eq $PwsVerChk){
        ...
        # $obj.Popup("PowerShell version is not matched. I'm ending the process.`r`n`r`n"+$_Exception.Message, 0, "Module Check", 0x30) | Out-Null
        # [int]$Button = $obj.Popup("実行中のPowerShellは"+$PwsVerChk+"です。`r`n`r`nこのスクリプトはバージョン"+$Yaml.PowerShell.Version+"でのみ動作確認しています。処理を続行しますか？", 0, "警告", 4) | Out-Null
        [int]$Button = $obj.Popup("The running PowerShell version is "+$PwsVerChk+".`r`n`r`nThis script has only been tested with version "+$Yaml.PowerShell.Version+". Do you want to continue?", 0, "WARNING", 4) | Out-Null
        switch($Button){
            6 { break } # OK(Continue)
            7 { Exit }  # Cancel(End)
