...
- **列フィルタリング**: 不要な内部カラムを除外（CreatedAt, RowError, RowState, Table, ItemArray, HasErrors）
...
`暗号化文字列の作成` フォルダー内の「`暗号化文字列の作成.ps1 - ショートカット.lnk`」を実行し、SQL Serverパスワードを暗号化
...
2. **暗号化パスワードファイル**: `mssql2022-dev-db.pass`
   - `暗号化文字列の作成` フォルダー内の「`暗号化文字列の作成.ps1 - ショートカット.lnk`」を実行
   - SQL Serverのパスワードを入力して暗号化ファイルを作成
   - 作成したファイルを `Excelの操作` フォルダーに配置
...
├── 暗号化鍵の作成/
│   └── 鍵ファイル作成.ps1 - ショートカット.lnk
├── 暗号化文字列の作成/
│   └── 暗号化文字列の作成.ps1 - ショートカット.lnk
├── 必要なモジュールの導入/
│   └── 指定モジュールの導入.ps1 - ショートカット.lnk
└── Excelの操作/
...
- ファイルパスが正しいか確認
- `暗号化鍵の作成` フォルダー内の「`鍵ファイル作成.ps1 - ショートカット.lnk`」を実行して鍵ファイルを生成
...
- 正しい鍵ファイルを使用しているか確認（作成時と同じ鍵が必要）
- `暗号化文字列の作成` フォルダー内の「`暗号化文字列の作成.ps1 - ショートカット.lnk`」を実行してパスワードファイルを再作成
...
- PSScriptAnalyzer準拠への対応
  - ホワイトスペースの統一（if、try、catch、else文の前後）
  - カンマの後のスペース追加
  - 演算子の前後のスペース統一
  - Test-ModuleAvailable関数にヘルプコメント追加
- コード品質の向上とベストプラクティス適用
