...
公開前に `audit-prepublic.ps1` を実行すると、  
ログやテスト結果、実在メールアドレスなどの混入を機械的にチェックできます。

```powershell
# 例: 実在ドメインを Forbidden としてチェックする場合
...
> **Note**: 実際の適用ルール・除外ルールの正本は [PSScriptAnalyzerSettings.psd1](PSScriptAnalyzerSettings.psd1) です。`LOG/`（配下含む）および検証成果物（例: `coverage.xml`, `testResults.xml`）は対象外として扱います。方針の背景は [ADR-0011](adr/0011-repository-wide-scriptanalyzer-policy.md) を参照してください。
...
- PSScriptAnalyzerのチェックを通過すること（Error/Warningレベルは必須）
  ...
  Informationレベルの扱いについては [ADR-0006](adr/0006-psscriptanalyzer-information-level.md) を参照。除外対象とルールの正本は [PSScriptAnalyzerSettings.psd1](PSScriptAnalyzerSettings.psd1) および [ADR-0011](adr/0011-repository-wide-scriptanalyzer-policy.md) を参照。
...
CI失敗時の判断は、  
[docs/Playbook.md](docs/Playbook.md)および **CI Failure Review Issue テンプレ**を用いて記録します。

詳細な設計判断については  
[ADR-0013: 定期（四半期）CIを環境変化検知専用とし、人間判断を前提とする](./30_ADR/ADR-0013-quarterly-ci-as-environment-signal.md)  
を参照してください。
